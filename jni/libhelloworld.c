...
/* Benchmark an helloworld OpenCL program, which takes an input char array of '1's, and writes an output 
 * char array of '2's.
 *
 * Read array_size as the first argument (or default to default_array_size).
 */
int benchmark_helloworld(
        int argc, 
        char* argv[],
        const char * kernel_src,
        unsigned int default_array_size,
        int check_output_array)
{
    ...
    if (argc < 2) {
        ...
        int result;
        result = sscanf(argv[1], "%" PRIu64, &array_size);
        if (result != 1) {
            printf("result == %d\n", result);
